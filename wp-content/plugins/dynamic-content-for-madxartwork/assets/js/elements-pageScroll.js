( function( $ ) {
    ...
    function removeScrollEffects(){
    		...
    		$('.madxartwork-'+settings_page.scrollEffects_id_page).removeClass('dce-pageScroll-element');
	...
	function animateSection() {
		...
		sectionsAvailable.each(function(){
			...
			// @p
			transformSection(actualBlock.children('.madxartwork-container'), animationValues[0], animationValues[1], animationValues[2], animationValues[3], animationValues[4]);
	...
	function initHijacking() {
		...
		visibleSection.children('.madxartwork-container').velocity(animationVisible, 1, function(){
			visibleSection.css('opacity', 1);
	    	topSection.css('opacity', 1);
	    	bottomSection.css('opacity', 1);
		});
        topSection.children('.madxartwork-container').velocity(animationTop, 0);
        bottomSection.children('.madxartwork-container').velocity(animationBottom, 0);
	}
    ...
    function prevSection(event) {
        ...
        if( !animating && !visibleSection.is(":first-child") ) {
            ...
            visibleSection.removeClass('dce-ps-visible').children('.madxartwork-container').velocity(animationParams[2], animationParams[3], animationParams[4])
            .end().prev(sezioni).addClass('dce-ps-visible').children('.madxartwork-container').velocity(animationParams[0] , animationParams[3], animationParams[4], function(){
            	animating = false;
    ...
    function nextSection(event) {
        ...
        if(!animating && !visibleSection.is(":last-of-type") ) {
            ...
            animating = true;
            visibleSection.removeClass('dce-ps-visible').children('.madxartwork-container').velocity(animationParams[1], animationParams[3], animationParams[4] )
            .end().next(sezioni).addClass('dce-ps-visible').children('.madxartwork-container').velocity(animationParams[0], animationParams[3], animationParams[4], function(){
            	animating = false;
            	//alert($.Velocity);
            	if( hijacking == 'off') $(window).on('scroll', scrollAnimation);
            });
	...
	function deviceType() {
		...
		var currentDevice = madxartworkFrontend.getCurrentDeviceMode();
	...
	var set_sectionHeight = function(){
		...
		if(sectionsAvailable.length) sectionsAvailable.each(function(){
			var childrenContainer = $(this).children('.madxartwork-container');
	...
	var WidgetElements_PageScroll = function(  ) {
		//alert('PageScroll');
	        	...
	        	sezioni = '.madxartwork-' + settings_page.scrollEffects_id_page + ' > .madxartwork-inner > .madxartwork-section-wrap > .' + $customClass;
				...
				// Elements
				sectionsAvailable.addClass('dce-ps-section').removeClass('madxartwork-section-boxed');
				$('.madxartwork-' + settings_page.scrollEffects_id_page + ' > .madxartwork-inner > .madxartwork-section-wrap > .'+$customClass+':first-of-type').addClass('dce-ps-visible');
		...
		//alert($('.madxartwork-'+settings_page.scrollEffects_id_page+ ' .madxartwork-inner').length);
		$('.madxartwork-'+settings_page.scrollEffects_id_page).addClass('dce-pageScroll-element');
		// $('.madxartwork-'+settings_page.scrollEffects_id_page).removeAttr('data-hijacking');
		// $('.madxartwork-'+settings_page.scrollEffects_id_page).removeAttr('data-animation');
	...
	$( document ).on( 'ready', function() {
		...
		if( typeof madxartworkFrontendConfig.settings.page !== 'undefined' ){
			//alert(madxartworkFrontendConfig.settings.page);
			...
			if( settings_page ){
				...
				if ( madxartworkFrontend.isEditMode() ){
					...
					madxartwork.settings.page.addChangeCallback( 'hijacking', handleScrollEffects_hijacking );
					madxartwork.settings.page.addChangeCallback( 'scrollEffects_height', handleScrollEffects_height );
